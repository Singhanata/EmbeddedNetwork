...
void NET_comm::non_callback(slot_t* s) {
  // check destination
  if (Header::checkaddress((*s).md.destination,
                           mac->device_address)) {  // check destination address
    ...
    // forward immediately
    if ((*s).md.destination.nP ==
        (*mac).device_address.nP) {        // check net prefix
      byte dID = (*s).md.destination.dID;  // check destination device ID
      byte nID = NET_comm::find_nexthop(
          dID);  // search net entry get nexthop device ID
      mac_header_t h;
...
void NET_comm::con_callback(slot_t* s) {
  if (Header::checkaddress((*s).md.destination,
                           mac->device_address)) {  // for this node
...
void NET_comm::fail_callback(mac_header_t* h, slot_t* s) {
  ...
  if (net_stat.route_eval <
      ROUTE_QUALITY_THRESHOLD) {  // evaluation below threshold
    net_stat.route_eval =
        ROUTE_QUALITY_THRESHOLD;  // set to min. route evaluation
  }
}
...
void NET_comm::set_pr_session(byte flag) {
  if (pr_session.isActive) {
    return;
  }                            // If PR session is on going, wait for finish
  ...
  if (flag & NET_STAT_PR_REQ) {
    ...
    NET_comm::beacon_create(
        &pr_session.beacon_holder,  // BEACON PR REQ records on slot
  ...
  pr_session.beacon_holder.content[len++] =
      ('0' + net_stat.bs_level);  // content 1 bs level
  pr_session.beacon_holder.content[len++] =
      ('0' + net_stat.child_count);  // content 2 length of children
  for (int i = 0; i < net_stat.child_count; i++) {
    pr_session.beacon_holder.content[len++] =
        subnode->id;  // record entry of child ID
  }
...
void NET_comm::send_response(slot_t* s) {
  ...
  Header::copyaddress(&h.sender,
                      &(*mac).device_address);  // this device address
  // set meta data
   ...
   * For example, given quota of 20 and 4 children -> each child is given 20/4 -
   * 1 = 4
   */
  ...
  // route record
  pr_session.beacon_holder.content[len++] =
      net_stat.bs_level;  // extract meaningful hop
  for (int i = 0; i < net_stat.bs_level; i++) {
    pr_session.beacon_holder.content[len++] =
        net_stat.selfRoute.hop[i];  // record each hop
  }
  (*s).md.len = len;
  if (mac->onSend(h, &pr_session.beacon_holder)) {  // successfully sent
    pr_session.beacon_holder.access_count =
        0;  // TODO: mark beacon holder vacant
  }
}
...
byte NET_comm::read_request(slot_t* s) {
  ...
  if (((*s).md.origin.nP != (*mac).device_address.nP) ||
      ((*s).md.origin.nP != bs_address.nP)) {
    return REQ_ERR_OUT_OF_DOMAIN;
  }                                        // check net domain
  ...
  if (bs_level > net_stat.bs_level) {      // this node is nearer to BS
    if (net_stat.child_quota >
        (child_count + 1)) {  // this node still have enough quota
      register_subnode((*s).md.origin.dID,
                       (*s).md.origin.dID);  // record sender as subnode
      for (int i = 0; i < child_count; i++) {
        register_subnode(
            (*s).content[1 + i],
            (*s).md.origin.dID);  // record all subnodes of sender as subnode
      }
...
byte NET_comm::read_response(slot_t* s) {
  ...
  if (!Header::checkaddress((*s).md.origin, mac->device_address)) {
    return RES_ERR_NOT_FOR_ME;
  }
  if ((*s).md.id != pr_session.id) {
    return RES_ERR_ID_MISMATCH;
  }
  ...
  if ((*s).md.len != len) {
    return RES_ERR_SIZE_MISMATCH;
  }  // check content size

  quota -= (net_stat.child_count +
            net_stat.child_quota);  // calculate remaining quota
...
void NET_comm::beacon_create(
    slot_t* s, address_t* destination, address_t* origin, byte type, byte id) {
  // write address
  Header::copyaddress(&(*s).md.destination,
                      destination);  // write intended receiver address
