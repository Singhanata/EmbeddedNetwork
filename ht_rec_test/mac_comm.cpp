...
bool MAC_comm::receive_callback(slot_t* s) {
  bool isToThis = Header::checkaddress(device_address,
                                       h_rx.recipient);  // check device address
  bool isReceiver =
      (isToThis ||
       Header::checkbroadcast(
           h_rx.recipient));  // check broadcast address & device address
...
bool MAC_comm::onSend(mac_header_t h, slot_t* s) {
  if ((*s).md.type & MSG_TYPE_CON == (*s).md.type &
      MSG_TYPE_CON) {  // CON message
...
void MAC_comm::mac_send_ack(slot_t* s) {
  ...
  Header::copyaddress(&h_ack.recipient,
                      &h_rx.sender);  // set recipient as sender of message
  Header::copyaddress(&h_ack.sender,
                      &device_address);  // set sender as device address
